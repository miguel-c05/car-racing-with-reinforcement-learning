"""Video recording script for trained PPO agents.

Records episodes of trained models to video files for visual analysis and
demonstration. Manually handles frame stacking and transposition to match
training preprocessing without using vectorized environments.

Usage:
    ```bash
    # Update best_model_path in script, then run:
    python record.py
    ```

Output:
    MP4 video files saved to videos/{TOTAL_TIMESTEPS}/episode_{N}.mp4
"""
...
if __name__ == "__main__":
    ...
    # Load the best model
    best_model_path = r"C:\car-racing-with-reinforcement-learning\models\phased\phase_2_refinement\best_model\best_model.zip"
